...
/**
 * Function to scrape a single page of projects.
 */
const scrapeSerbiaProjectsPage = async (url) => {
  try {
    ...
    const nextPageUrl = url.replace(`b_offset=${currentOffset}`, `b_offset=${currentOffset + 10}`);

    // Check if the next page URL is valid by looking for the "Next page" link
...
const scrapeSerbiaProjects = async () => {
  ...
  while (currentPageUrl) {
    ...
    const { projects, nextPageUrl } = await scrapeSerbiaProjectsPage(currentPageUrl);
 ...
 */
const scrapeAndPrintSerbiaProjects = async () => {
  const projects = await scrapeSerbiaProjects();

  console.log('----- Scraped Projects for Serbia -----');
