...
/**
 * Function to scrape a single page of projects for Croatia.
 */
const scrapeCroatiaProjectsPage = async (url) => {
  try {
...
const scrapeCroatiaProjects = async () => {
  ...
  while (currentPageUrl) {
    ...
    const { projects, nextPageUrl } = await scrapeCroatiaProjectsPage(currentPageUrl);
 ...
 */
const scrapeAndPrintCroatiaProjects = async () => {
  const projects = await scrapeCroatiaProjects();

  console.log('----- Scraped Projects for Croatia -----');
